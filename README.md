...
- 支持长短信自动合并（30秒超时）
- 支持短信兜底通道：网络推送和邮件全部失败时，用设备 SIM 卡直接转发到兜底号码
- 支持管理员短信远程发送短信和重启设备
- 支持运行期修改 WiFi；开机连续连接失败会进入 `SMS-Setup-XXXX` 应急配网热点
- 支持在 Web 后台关闭 ESP32 板载状态灯，避免长期亮灯/闪烁
- 支持eSIM卡管理功能

## 管理员短信命令

在 Web 后台配置管理员手机号后，该号码可发送以下短信命令：

| 命令 | 作用 | 示例 |
|---|---|---|
| `SMS:号码:内容` | 用设备 SIM 卡主动发短信 | `SMS:13800138000:测试` |
| `WIFI:名称:密码` | 远程切换 WiFi，成功才保存，失败回滚 | `WIFI:MyHome:secret123` |
| `PING` / `/PING` | 短信回执设备状态（WiFi/IP/信号/4G/时间） | `PING` |
| `RESET` | 重启模组和 ESP32 | `RESET` |

## WiFi 配网与短信兜底

- WiFi SSID/密码可在 Web 后台修改，保存后设备会尝试连接新 WiFi。
- 如果开机连续 3 次连接 WiFi 失败，设备会开启 `SMS-Setup-XXXX` 热点，密码 `12345678`，连接后访问 `http://192.168.4.1` 填写新 WiFi。
- `code/wifi_config.h` 仍作为首次烧录或 NVS 为空时的默认 WiFi 配置。
- 可在 Web 后台填写“短信兜底接收号”。收到短信后，如果已配置网络通道但所有推送/邮件都失败，设备会把短信截断到单条短信长度后用 SIM 卡发给兜底号码。留空即关闭。
